...
With a false positive, defenses will be expecting a pass when it's actually a run. This means that they will generally have smaller, faster players on the field (defensive backs and coverage linebackers in place of lineman and run stopping linebackers). This will give the offense a greater chance for a successful play (one that gets them a first down or keeps them on track to get one). It will also likely increase their risk of giving the offense an explosive play (one that is 20+ yards) via the run. It is likely that the swap in players will yield a more successful pass rush. 

With a false negative, defenses will be expecting a run when it's actually a pass. This means that they will generally have larger, slower players on the field (more defensive lineman and linebackers in place of defensive backs and coverage linebackers). This will give the offense a greater chance for a successful play passing. It will also likely slightly increase the risk of giving the offense an explosive play via the pass compared to a false positive.

While most defensive coaches are generally risk adverse and would avoid the slightly increased risk of an explosive play, the risks of both are fairly balanced. Letting your opponent slowly move the ball down the field versus the risk of letting them move the ball down the field more quickly via the pass - neither is particularly compelling. In the end, it's a personal choice based on the defensive coaching, which favors using accuracy as the metric by which we'll gauge the models.
...
We were able to achieve a 23% increase over our baseline of 60.38% which exceeded our expectations. However, while the accuracy of the models was an improvement over the baseline, it's still below what is typically expected of a model. To further validate these models, we're going to run them through a series of real world scenarios and compare their accuracy against a baseline value (the highest likelihood of either a run or a pass occuring). This will determine how accurate our model is compared with random chance. We will consider anything below the baseline to be underperforming while anything above the baseline is outperforming. Anything that is 15% higher than the baseline will be considered "significantly outperforming". 

When evaluating the models accuracy for each of the teams, Gradient Boosting produced the best accuracy with team TB at 83% with seven teams boasting an accuracy score of 79% or more (across multiple models). To further test whether tenanted models would perform better than a general model, tenanted models were created for the team with the highest and lowest "best" accuracy across all models (TB at 83% and GB at 68%). Tenanted models were created for each by running parameter tuning and model training using only that dataset. This severely limited the amount of data available for the exercise, making this largely toy models, however, the results were interesting. 
 - Raising the Floor: Using `Team=GB` yielded a best accuracy of 0.6880 from LogisticRegression. This was a slight increase (0.5%) when compared to the model with the best accuracy for `Team=GB` (Gradient Boost at 0.6833). 
 - Raising the Ceiling: Using `Team=TB` yielded a best accuracy of 0.8113 from LogisticRegression. This was a slight decrease (2%) when compared to the model with the best accuracy for `Team=TB` (Gradient Boost at 0.8323).
While the data here didn't provide a definitive answer, further research would be warrented here with larger datasets. 

This leads to the conclusion that an ensemble approach potentially coupled with tenanted models would yield the best accuracy for all scenarios and might drive the accuracy into the low to mid 80s. 
