...
When calculating accuracy for each team, Gradient Boosting produced the best accuracy with team TB at 0.8323 with seven teams boasting an accuracy score of 79% or more. While there is natural variance with any data subset, this does beg the question if tenented models would outperform a single general model. Also, as the best performing model (GradientBoostingClassifier) wasn't able to outperform every scenario. This leads to the conclusion that an ensemble approach (coupled with tenented modesl) would yield the best accuracy for all scenarios and drive the accuracy into the low to mid 80s. 
